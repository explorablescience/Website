...
function SimulationLogic({ canvasRef, temperature, coupling, drawFireflies, isVisible }: {
    ...
    useEffect(() => {
        ...
        if (!canvas) logger.error(new Error("Canvas reference is null in the One Firefly simulation (Science of Complexity article)"));
        const cv = canvas!;
        ...
        if (!context) logger.error(new Error("Canvas context is null in the One Firefly simulation (Science of Complexity article)"));
        const ctx = context!;
