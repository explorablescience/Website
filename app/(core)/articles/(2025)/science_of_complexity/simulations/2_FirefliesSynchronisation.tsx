...
function SimulationLogic({ canvasRef, aspectRatio, temperature, coupling, drawFireflies, omegaActivated, fakeSynchronisation, conclusion, isVisible }: {
    ...
    useEffect(() => {
        ...
        if (!canvas) logger.error(new Error("Canvas reference is null in the Fireflies Synchronisation simulation (Science of Complexity article)"));
        const cv = canvas!;
        ...
        if (!context) logger.error(new Error("Canvas context is null in the Fireflies Synchronisation simulation (Science of Complexity article)"));
        const ctx = context!;
