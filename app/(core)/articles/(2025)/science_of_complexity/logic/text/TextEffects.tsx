...
export function ASide(props: JSX.IntrinsicElements['div'] & { data: typeof ASidesData[keyof typeof ASidesData] }) {
    ...
    return (
        <div className={`${styles.aside} ${data.title.toLowerCase()}`} style={{ borderLeft: `4px solid ${data.color}` }}>
...
export function Citation(props: JSX.IntrinsicElements['div']) {
    return (
        <div className={styles.citation}>
            <div className={styles['citation-left']}>
                <Image src={quote_open} alt="Open quote" />
            </div>
            <div className={styles['citation-text']}>
                {props.children}
            </div>
            <div className={styles['citation-right']}>
...
export function IColor(props: { type: string, bold?: boolean, italic?: boolean, children: React.ReactNode }) {
    return <>
        <span className={`${styles[`text-color-${props.type}`]} ${props.bold ? styles['text-color-bold'] : ''} ${props.italic ? styles['text-color-italic'] : ''}`}>
            {props.children}
        </span>
    </>
}
...
export function GradientText(props: { from: [number, number, number], to: [number, number, number], bold?: boolean, italic?: boolean, children: string }) {
    ...
    return (
        <span className={`${styles['gradient-text']} ${props.bold ? styles['gradient-text-bold'] : ''} ${props.italic ? styles['gradient-text-italic'] : ''}`}>
            {gradientLetters}
        </span>
    );
}
...
export function MoreInformations(props: { title: string, children: React.ReactNode }) {
    ...
    return (
        <section className={styles['more-informations']}>
            <div className={styles['more-informations-title']}>
                <div className={styles['more-informations-label']}>
                    <span>For more informations...</span>
                </div>
                <h3 className={styles['more-informations-title-content']}>{props.title}</h3>
                <div className={styles['more-informations-icon']} onClick={expandToggle}>
            ...
            {isExpanded && <div className={styles['more-informations-bar']} />}
            <div className={`${styles['more-informations-content']} ${isExpanded ? styles['more-informations-content-open'] : ''}`} ref={moreInfosRef} style={{ height: isExpanded ? `100%` : "0px" }}>
                {props.children}
            </div>
        </section>
    );
...
export function TheoristQuestions(props: { children: React.ReactNode }) {
    return (
        <section className={styles['theorist-questions']}>
            <div className={styles['theorist-questions-title-bar'] + ' ' + styles['theorist-questions-title-bar-up']}></div>
            <div className={styles['theorist-questions-title']}>
                <div className={styles['theorist-questions-title-content']}>Theorist Corner</div>
            </div>
            <div className={styles['theorist-questions-content']}>
                {props.children}
            </div>
            <div className={styles['theorist-questions-title-bar'] + ' ' + styles['theorist-questions-title-bar-down']}></div>
        </section>
    );
}

export function Separator(props: JSX.IntrinsicElements['div']) {
    return <div className={styles['separator']} {...props} />;
}
...
export function CheckList(props: { items: { label: string, checked: boolean, description?: string }[] }) {
    ...
    for (const item of props.items) {
        ...
        items.push(
            <li key={item.label} className={styles['checkbox-item']}>
                <div className={item.checked ? styles['checkbox-yes'] : styles['checkbox-no']}>
                ...
                <div>
                    {!item.checked && <div className={styles['crossbar']} />}
                    <span style={{ color }}>{item.label}</span>
                    {item.description && <span className={styles['description']}> - <i>{item.description}</i></span>}
                </div>
            </li>
        );
    }
...
export function GraphList(props: { items: JSX.Element[], customWidth?: { arrow: string, bar: string } }) {
    return <>
        <div className={styles['graph-list']}>
            <div className={styles['graph-list-arrow']} style={props.customWidth ? { width: props.customWidth.arrow } : {}}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill={"#6a5ff7"}>
                    <polygon points="50 15, 100 100, 0 100" />
                </svg>
            </div>
            <div className={styles['graph-list-bar']} style={props.customWidth ? { width: props.customWidth.bar } : {}}></div>
            <ol>
                {props.items.map((item, index) => (
                    <li key={index} className={styles['graph-list-item']}>
                        {item}
                    </li>
                ))}
            </ol>
    ...
    return (
        <section className={styles['more-informations']}>
            <div className={styles['more-informations-title']}>
                ...
                <h3 className={styles['more-informations-title-content']}>Sources</h3>
                <div className={styles['more-informations-icon']} onClick={expandToggle}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" style={{ transform: isExpanded ? "rotate(0deg)" : "rotate(90deg)", transition: "transform 0.3s ease-in-out" }}>
                        <path fill="currentColor" d="M19 6.25l-1.5-1.5-7.5 7.5-7.5-7.5L1 6.25l9 9 9-9z"></path>
                    </svg>
                ...
                <Paragraph>
                    <ul>
                        {props.list.map(([title, link, description], index) => (
                            <li key={index} className={styles['source-item']}>
                                <div className={styles['source-item-content']}>
